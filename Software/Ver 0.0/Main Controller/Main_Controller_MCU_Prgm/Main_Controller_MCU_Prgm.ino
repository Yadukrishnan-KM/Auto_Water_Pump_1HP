...
int brightValue = 1023;                         // LCD BAcklight brightness powerSymbol
...
// home screen updation variables
...
// ****************************************************************************************************************************************************************************************************
 ...
 // checking waterlevel for setting and resetting the flags if auto selected (for tuning ON and OFF the pump)
   if(waterLvl == 10 || waterLvl == 20) {
     if(pumpOperationModeSetting)  waterLvlLowFlag = true;
   }
   else if (waterLvl == 100) {                                                       // tank full disable all the motor on flags
     waterLvlLowFlag = false;
     manualOnFlag = false;
     scheduleOnFlag = false;
     currentCounterTimePumpSchedule = 0;
...
if(pumpScheduleTimeSetting != 1){
  ...
  if((currentCounterTimePumpSchedule - previousCounterTimePumpSchedule) >= pumpScheduleTime) scheduleOnFlag = true;
...
// ************************************************************************************************************************************************************************************************************
 // checking for transmitter timeout of 2 min, timer reset inside ISR
 ...
 if((currentCountertransmitterTimeout - previousCounterTransmitterTimeout) >= 120000){
...
// *************************************************************************************************************************************************************************************************************
 ...
 if(pumpRunningError || transmitterError || batLow){
...
// reset section after 2 days for resfreshing everything if motor not running otherwise waiting for pump off

currentMillisRest = millis();
//                                                                                          Serial.println(currentMillisRest);    
if(!motorStateMemory){
   if(currentMillisRest >= 172800000){  //172800000 in millisecond for 2 days
