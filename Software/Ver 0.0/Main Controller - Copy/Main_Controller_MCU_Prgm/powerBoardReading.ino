...
if(motorPinStatusFlag){                                                // only check if motor is in run condition
  ...
  dryRun = digitalRead(dryRunPin);                                     // read current only if motor is on
  if (dryRun){
  ...
  if((currentMills0 - previousMills0) >= 30000) dryRunFlag = true;     // time period of 30 sec for ensuring that it is dry run. detect bt low current draw(by pcb hardware)
   ...
   dryRunFlag = false;
                                                                               ...
                                                                               Serial.println("dryrun inside last else normal");
...
// low volt section, check always, give delay if motor is on
 ...
 lowVolt = digitalRead(lowVoltPin);
 ...
 voltPresent = digitalRead(voltPresentPin);
 if(motorPinStatusFlag){
  ...
  if(!voltPresent) {
    ...
    if(voltPresent){
      ...
      if((currentMills3 - previousMills3) >= 20000) {                          // pump is waiting to turn off for 20 sec after a powewrfailure in running condition
        voltPresentFlag = true;
        pwrFailFlag = false;
    }
